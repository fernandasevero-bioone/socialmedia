...
async function publishToPage(account, caption, mediaUrls) {
  ...
  if (img) {
    ...
    return { ok: true, externalUrl: `https://facebook.com/${r.post_id || r.id}` };
  }
  const r = await gpost(`${GRAPH}/${account.pageId}/feed`, { message: caption, access_token: account.token });
  return { ok: true, externalUrl: `https://facebook.com/${r.id}` };
}
...
async function publishToInstagram(account, caption, mediaUrls) {
  ...
  return { ok: true, externalUrl: `https://instagram.com/${account.username || ''}`, id: pub.id };
}

module.exports = { isConfigured, absMedia, authUrl, exchangeCode, getConnections, publishToPage, publishToInstagram, SCOPES };
