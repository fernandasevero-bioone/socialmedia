...
// Remove Meta (Facebook + Instagram) connections for whoever owns the given
// Facebook user id — used by the deauthorize / data-deletion callbacks.
function purgeMetaUser(metaUserId) {
  const db = load();
  const affected = [];
  for (const [fid, accts] of Object.entries(db.accounts)) {
    const match = (accts.facebook && accts.facebook.fbUserId === metaUserId) ||
                  (accts.instagram && accts.instagram.fbUserId === metaUserId);
    if (match) { delete accts.facebook; delete accts.instagram; affected.push(fid); }
  }
  if (affected.length) save(db);
  return affected;
}
...
module.exports = { getAccounts, setAccount, removeAccount, getPosts, addPost, getPost, updatePost, updatePostById, deletePost, getDueScheduled, purgeMetaUser, getUsers, getUserByEmail, addUser, updateUser, deleteUser };
