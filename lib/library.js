...
function create({ title, category, caption, image, media, platforms }) {
  const doc = loadDoc();
  const mediaArr = Array.isArray(media) && media.length ? media : (image ? [image] : []);
  const post = {
    id: 'lib-' + crypto.randomBytes(4).toString('hex'),
    category: category || '',
    title: title || 'Untitled post',
    image: mediaArr[0] || '/img/posts/announcement.svg', // cover
    media: mediaArr.length ? mediaArr : ['/img/posts/announcement.svg'],
    caption: caption || '',
...
function update(id, fields) {
  ...
  if (!post) return null;
  for (const key of ['title', 'category', 'caption', 'platforms']) {
    if (fields[key] !== undefined) post[key] = fields[key];
  }
  // media array drives the cover image
  if (fields.media !== undefined) {
    const m = Array.isArray(fields.media) ? fields.media : [];
    post.media = m.length ? m : post.media;
    post.image = post.media[0] || post.image;
