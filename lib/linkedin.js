...
async function exchangeCode(code, redirectUri) {
  ...
  console.log('[linkedin] token granted scopes:', json.scope || '(none reported)');
...
async function getOrganizations(token) {
  ...
  console.log('[linkedin] admin organizations found:', orgs.length, orgs.map(o => o.name));
  return orgs;
}
...
async function publishToOrg(account, caption, mediaUrls) {
  ...
  if (img) {
    ...
    } catch (e) { console.log('[linkedin] image upload failed, posting text-only:', e.message); }
  }
  console.log('[linkedin] publishing to', account.orgUrn);
  const r = await lpost('/posts', account.token, body);
  const postUrn = r.headers.get('x-restli-id') || (r.json && r.json.id) || '';
  console.log('[linkedin] post created:', postUrn || '(no id returned)');
  return {
    ok: true,
    platformId: postUrn,
