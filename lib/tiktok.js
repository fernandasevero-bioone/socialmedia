...
// Trim: values pasted into a hosting dashboard often carry stray whitespace or
// newlines, which makes TikTok reject the credential as invalid.
function clientKey()    { return (process.env.TIKTOK_CLIENT_KEY || '').trim(); }
function clientSecret() { return (process.env.TIKTOK_CLIENT_SECRET || '').trim(); }

function isConfigured() {
  return !!(clientKey() && clientSecret());
}

// Masked view of the configured credentials, for troubleshooting the
// "client_key" error without exposing secrets.
function debugInfo() {
  const k = clientKey(), s = clientSecret();
  const mask = v => v ? `${v.slice(0, 4)}…${v.slice(-2)} (len ${v.length})` : '(not set)';
  return {
    clientKey: mask(k),
    clientSecretSet: !!s,
    clientSecretLength: s.length,
    rawKeyHadWhitespace: (process.env.TIKTOK_CLIENT_KEY || '') !== k,
    rawSecretHadWhitespace: (process.env.TIKTOK_CLIENT_SECRET || '') !== s
  };
}
...
module.exports = { isConfigured, debugInfo, authUrl, exchangeCode, refreshAccessToken, getValidToken, getUserInfo, publish, SCOPES };
